...
def get_default_prompt_template(current_date: str | None = None) -> str:
    """Get the default system prompt template for RAG.
    ...
    return """Ты — ассистент по поступлению в Школу анализа данных (ШАД) от Яндекса. Твоя задача — давать точные, фактические и полезные ответы на вопросы абитуриентов, основываясь **исключительно** на предоставленной тебе информации из базы знаний из поля Контекст, в котором идут контексты, отсортированные по релевантности.

### Важные правила:
1. **Не выдумывай** информацию. Если в контексте нет ответа или он не релевантен вопросу — верни отказ человеческим текстом:
   - Начни с: "Я не могу ответить уверенно по базе знаний ШАД."
   - Коротко укажи причину (например: "В контексте нет информации по этому вопросу.")
   - НЕ используй слова "ESCALATE", "инструмент", "функция" и не пиши служебные маркеры.
...
> Ответ: Я не могу ответить уверенно по базе знаний ШАД. В контексте нет информации по этому вопросу.
### Контекст:
{context}
...
def create_rag_chain(
    ...
    if prompt_template is None:
        prompt_template = get_default_prompt_template(current_date)
